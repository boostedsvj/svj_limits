...
def main():
    ...
    idir_rinj0 = '/uscms/home/bregnery/work/CMSSW_11_3_4/src/boosted/svj_limits/self_test/toyfits_Oct01/fitDiagnosticsObserveddc_SVJ_s-channel_mMed-'
    idir_rinj1 = '/uscms/home/bregnery/work/CMSSW_11_3_4/src/boosted/svj_limits/toyfits_Oct01/fitDiagnosticsObserveddc_SVJ_s-channel_mMed-'
    ...
    for i, (f_rinj0, f_rinj1) in enumerate(zip(path_rinj0, path_rinj1)):
        ...
        histo_rinj0.Draw()
        canvas.SaveAs(f"siginj_rinj0_MZ_{mz[i]}_GeV.pdf")
        canvas.SaveAs(f"siginj_rinj0_MZ_{mz[i]}_GeV.png")
        
        histo_rinj1.Draw()
        canvas.SaveAs(f"siginj_rinj1_MZ_{mz[i]}_GeV.pdf")
        canvas.SaveAs(f"siginj_rinj1_MZ_{mz[i]}_GeV.png")
    ...
    plt.title('Self Test: Mean of Gaussian Fit to (r-$r_{inj})$/rErr', fontsize=15)
    plt.ylim(-1.5, 1.5)
    plt.fill_between(plt.xlim(), 0.5, -0.5, color='#f88379', alpha=0.25, label='$\pm 0.5$')
    plt.savefig('self_pull_mean.png')
    plt.savefig('self_pull_mean.pdf')
    plt.close()
    ...
    plt.title('Self Test: $\sigma$ of Gaussian Fit to (r-$r_{inj})$/rErr', fontsize=15)
    plt.ylim(0.5, 2.5)
    plt.savefig('self_pull_stdev.png')
    plt.savefig('self_pull_stdev.pdf')
    plt.close()
